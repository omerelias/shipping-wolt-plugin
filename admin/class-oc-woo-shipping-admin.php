...
class Oc_Woo_Shipping_Admin {
	...
	public function setup_settings() {
		...
		$group_options = array(
			...
			'closing_dates',
            'price_depending',
			// new settigns for Meatchoice , allow validation for time slots if time slots are empty
			'allow_checkout_validation_without_slots',           
			'checkout_warning_message_without_slots',           
		);
	...
	public function woocommerce_order_formatted_billing_address( $raw_address, $order ) {
		...
		if (isset( $raw_address['city'])) {
			if (is_numeric($raw_address['city']) || ocws_is_hash($raw_address['city'])) {
				$city = ocws_get_city_title( $raw_address['city'] );
				if ($city) {
					$raw_address['city'] = $city;
				}
			}
		}
		return $raw_address;
